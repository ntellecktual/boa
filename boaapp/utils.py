...
def _get_video_paths(audio_file_instance):
    ...
    try:
        ...
        is_expected_structure = (
            len(parts) == 3
            and not relative_audio_path.is_absolute()
            and parts[0] == 'audio'
            and relative_audio_path.suffix.lower() == '.mp3'
        )
        ...
        if is_expected_structure:
            ...
            logger.debug(
                f'[_get_video_paths] Determined video dir: {video_output_dir_abs}, video path: {video_path_abs}, sync path: {sync_file_path}'
            )
            ...
            logger.warning(
                f'[_get_video_paths] Unexpected audio path structure: {relative_audio_path_str}'
            )  # Log original string
            ...
            if len(parts) == 0 or parts[0] != 'audio':
                logger.warning(
                    f"[_get_video_paths] Reason: First part is not 'audio' (or path is empty). Parts: {parts}"
                )
            if relative_audio_path.suffix.lower() != '.mp3':
                logger.warning(
                    f"[_get_video_paths] Reason: File suffix is '{relative_audio_path.suffix}', expected '.mp3'."
                )
        ...
        logger.error(
            f"[_get_video_paths] Error determining video paths for audio PK {audio_file_instance.pk} (path='{path_str}'): {e}",
            exc_info=True,
        )
        return None, None, None
